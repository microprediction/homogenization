...
'''
    ...
    <p>It decays at the rates of the chain and is solved order by order in a Schur basis of $Q_0$ on the mean-zero
    subspace, by back substitution through the triangular form, in the span of $\tau^k e^{\mu\tau}$. A generator
    with a Jordan block has no eigenvector basis, but it has a triangular one, and the powers of $\tau$ come out of
    the back substitution. The result is $a_i = s\,(1 + w_i + \eta_i)$, with error $O(\varepsilon^{N+1})$ after $N$
    orders.</p>
...
def fast_factor_page():
    ...
    $$\begin{aligned}
    \log s(t) = &\int_0^t \bar g \;-\; \delta\int_0^t c\,(\sigma_0g_1 + \sigma_1g_2) \\
    ...
    <div class="equation-card">
    $$\begin{aligned}
    \log s(t) = \;&-\kappa\theta_0\,I_1 + \tfrac12(\sigma_0^2 + \sigma_1^2)\,I_2 \\
      &+ \sqrt2\,\rho\,\delta\,\Big(\kappa\theta_1\sigma_0\,I_2 - \big(\sigma_0^2\sigma_1 + \tfrac12\sigma_1^3\big)\,I_3\Big) \\
      &+ \delta^2\Big(\kappa^2\theta_1^2\,I_2 - 2(1 + \rho^2)\,\kappa\theta_1\sigma_0\sigma_1\,I_3 \\
      &\qquad\quad + \big((1 + 3\rho^2)\,\sigma_0^2\sigma_1^2 + (\tfrac14 + \rho^2)\,\sigma_1^4\big)\,I_4\Big) + O(\delta^3),
    \end{aligned}$$
    $$\begin{aligned}
    ...
    <div class="equation-card">
    $$\begin{aligned}
    u(t,x,y) = \;&e^{-B x}\,s(t)\,\Big(1 + \varepsilon\Big[\big(-\kappa\theta_1 B + \sigma_0\sigma_1 B^2\big)\,y + \tfrac14\sigma_1^2 B^2\,(y^2 - 1)\Big] \\
      &- \sqrt2\,\rho\,\varepsilon^{3/2}\Big[\big(-\kappa\theta_1\sigma_1 B^2 + \tfrac32\sigma_0\sigma_1^2 B^3\big)\,y
        + \tfrac14\sigma_1^3 B^3\,(y^2 - 1)\Big] + O(\varepsilon^2)\Big),
    \end{aligned}$$
    </div>
    <p>with $\log s(t)$ from the card above. The dependence on the current factor value $y$ first appears at order
    $\varepsilon$.</p>
    <p>At $t = 1$, $y = 0.3$, $\varepsilon = 0.0025$ and the parameters above:</p>
