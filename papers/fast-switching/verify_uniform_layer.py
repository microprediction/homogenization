...
1. If q(0) != 0, the first-order *outer* approximation has only O(eps)
   maximum error on [0, T], although its error is O(eps**2) at fixed T > 0.
   Adding the leading layer makes the error uniformly O(eps**2).
...
from uniform_layer import first_order, second_order_zero_start
...
def nonzero_start():
    ...
    q0 = q(0.0)
    outer_errors, composite_errors, fixed_errors = [], [], []
    rows = []
    for eps in EPSILONS:
        ...
        comp = first_order(grid, eps, ib, iq2, qv, q0, iql)
        ...
        rows.append(dict(epsilon=float(eps), outer_sup=oe, composite_sup=ce, outer_fixed_T=fe))
    return rows, order(outer_errors), order(composite_errors), order(fixed_errors)
...
def zero_start():
    ...
    for eps in EPSILONS:
        ...
        ib, iq2, _ = integrals(grid, eps, bv, qv)
        exact = exact_curve(eps, b, q, grid)
        omega_outer = eps * qv / 2 - eps ** 2 * qpv / 4
        outer = np.exp(ib + eps * iq2 / 2 - eps ** 2 * qv ** 2 / 8) * (1 + omega_outer)
        comp = second_order_zero_start(grid, eps, ib, iq2, qv, qpv, qp0)
        oe = float(np.max(np.abs(outer - exact)))
        ce = float(np.max(np.abs(comp - exact)))
        fe = float(abs(outer[-1] - exact[-1]))
...
def main():
    nz, nz_outer, nz_comp, nz_fixed = nonzero_start()
    z, z_outer, z_comp, cross = zero_start()
    print('1. q(0) != 0: first-order outer versus uniform composite')
    for row in nz:
    ...
    print(f'   observed orders: outer sup {nz_outer:.6f}, composite sup {nz_comp:.6f}, fixed-T outer {nz_fixed:.6f}')
    ...
    assert 1.85 < nz_comp < 2.15
    assert 1.85 < nz_fixed < 2.15
    assert 1.85 < z_outer < 2.15
    assert 2.8 < z_comp < 3.2
    assert max(cross) - min(cross) < 1e-12
    out = dict(nonzero_start=dict(rows=nz, outer_sup_order=nz_outer,
                                  composite_sup_order=nz_comp, outer_fixed_order=nz_fixed),
