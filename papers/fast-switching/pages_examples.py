...
""" + (("    <p>Here</p>\n    $$" + g_desc[1:-1] + " .$$\n    <p>") if (g_desc.startswith('$') and g_desc.endswith('$') and '$' not in g_desc[1:-1]) else ("    <p>where " + g_desc + ". ")) + r"""The average $\bar g$ is what the <a href="./idea.html">averaged model</a> sees. The
    ...
    <h3>First order</h3>
    <p>To first order in $\varepsilon$, at fixed $t > 0$,</p>
    ...
    <p>with the upper sign for a start in regime 1. If $\tilde g(0)=0$, the remainder is also uniform for
    $0\leq t\leq T$. Otherwise the uniform composite formula replaces $\tilde g(t)$ in the bracket by</p>
    $$\tilde g(t)-\tilde g(0)e^{-2t/\varepsilon}$$
    <p>and includes $-\varepsilon\tilde g(0)\int_0^t\tilde g(s)e^{-2s/\varepsilon}ds/2$ in the exponent. Its remainder is
    $O(\varepsilon^2)$ uniformly on every fixed bounded maturity interval. The proof and the exact crossover scale
    are on the <a href="./layers.html">initial-layers</a> page. The three outer factors have separate meanings,
    explained on the <a href="./idea.html">idea</a> page.</p>
    <ul>
...
""") + r"""      <li>The bracket is the memory of the starting regime. The switch stays near its starting state for about
    ...
    <p>The series does not satisfy $\omega(0) = 0$.</p>
    <p>An <a href="./layers.html">initial layer</a> of width about
    $1/(2\lambda)$ repairs the start and is solved order by order in the same way. Its leading term adds
    $\mp\tfrac12\varepsilon\,\tilde g(0)\,e^{-2\lambda t}$ to the first-order bracket, which returns the bracket to $1$
    at $t = 0$; when $\tilde g(0) \ne 0$ the first-order formula above is therefore accurate to $O(\varepsilon^2)$ only
    at fixed $t > 0$. The <a href="./engine.html">engine</a> page gives the version for any number of regimes.</p>
"""
    if extra:
...
def credit_page():
    ...
    <h2>The quantity</h2>
    <p>The survival of each company and of both, to time $t = 3$:</p>
    $$S_c(t) = \mathbb{E}\Big[e^{-\int_0^t (c_1 x_1 + c_2 x_2)\,dr}\Big], \qquad c = (1,0),\ (0,1),\ (1,1).$$
    <p>The default probabilities follow from them:</p>
    $$p_1 = 1 - S_{(1,0)}, \qquad p_2 = 1 - S_{(0,1)}, \qquad p_{12} = 1 - S_{(1,0)} - S_{(0,1)} + S_{(1,1)} .$$
    <p>The correlation of the two default indicators is</p>
    $$\operatorname{corr} = \frac{p_{12} - p_1 p_2}{\sqrt{p_1(1 - p_1)\,p_2(1 - p_2)}} .$$
...
'''
    ...
    $$\begin{aligned}
    \phi_{1,2}(u) = \;&\exp\Big(v_0\,D + \kappa\bar\theta\,I_1 + \frac{\varepsilon}{2}\,\kappa^2\tilde\theta^2\,I_2
      - \frac{\varepsilon^2}{8}\,\kappa^2\tilde\theta^2\,D^2\Big) \\
    &\times\Big(1 \pm \frac{\varepsilon}{2}\,\kappa\tilde\theta\,D \mp \frac{\varepsilon^2}{4}\,\kappa\tilde\theta\,D'\Big)
      + O(\varepsilon^3),
    \end{aligned}$$
    $$\begin{aligned}
    ...
    <p>Here $D = D(T)$ is the coefficient of the starting variance in the log characteristic function, the same as in
    Heston&apos;s model. $I_1$ and $I_2$ are the closed forms of $\int_0^T D$ and $\int_0^T D^2$. The upper sign is for a
    start in regime 1. The initial layer adds a term of size $\varepsilon^2 e^{-2\lambda T}$ to the bracket, which is
    below $10^{-10}$ here.</p>
    ...
    <table class="impl">
      ...
      <tbody>
        ...
        <tr><td>$D$, $D'$</td><td>&minus;0.256566 + 0.020515 i, &nbsp; &minus;0.070925 + 0.017241 i</td></tr>
        <tr><td>$I_1$, $I_2$</td><td>&minus;0.171410 + 0.009570 i, &nbsp; 0.034359 &minus; 0.004175 i</td></tr>
        <tr><td>averaged exponent $\kappa\bar\theta I_1$</td><td>&minus;0.0188551 + 0.0010527 i</td></tr>
        <tr><td>first-order exponent $\frac\varepsilon2\kappa^2\tilde\theta^2 I_2$</td><td>8.418e-6 &minus; 1.023e-6 i</td></tr>
        <tr><td>second-order exponent $-\frac{\varepsilon^2}8\kappa^2\tilde\theta^2 D^2$</td><td>&minus;4.006e-7 + 6.45e-8 i</td></tr>
        <tr><td>memory, first order $\frac\varepsilon2\kappa\tilde\theta D$</td><td>&minus;8.980e-4 + 7.18e-5 i</td></tr>
        <tr><td>memory, second order $-\frac{\varepsilon^2}4\kappa\tilde\theta D'$</td><td>1.241e-5 &minus; 3.02e-6 i</td></tr>
      </tbody>
    </table>
    </div>
